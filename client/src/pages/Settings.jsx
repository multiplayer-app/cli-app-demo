...
import {
  Settings2, Bell, Shield, Save, Check, AlertTriangle, Trash2,
  User, Key, Download, Upload, Copy, RefreshCw, Eye, EyeOff, Plus, X,
} from 'lucide-react'
import './Settings.css'
...
export default function Settings() {
  ...
  const createKey = () => {
    ...
    setApiKeys(prev => [...prev, { id: Date.now(), name: newKeyName, key, created: new Date().toISOString().split('T')[0], visible: true }])
    setNewKeyName('')
  ...
  const handleReset = () => {
    setSettings({
      siteName: 'My Dashboard', siteUrl: '', timezone: 'UTC', language: 'en',
      emailNotifications: true, pushNotifications: false, weeklyReport: true, marketingEmails: false,
      twoFactor: false, sessionTimeout: '30', ipWhitelist: '', displayName: '', profileEmail: '', bio: '', theme: 'dark',
    })
    ...
    <div className="settings-page">
      ...
      <div className="page-header">
        ...
        <div className="header-actions">
          <button className="btn" onClick={handleImportSettings}><Upload size={14} /> Import</button>
          <button className="btn" onClick={handleExportSettings}><Download size={14} /> Export</button>
        ...
        {activeTab === 'profile' && (
          <div className="settings-section">
            ...
            <div className="settings-form">
              ...
              <div className="form-row">
                <label>Email</label>
                <input type="email" value={settings.profileEmail} onChange={e => update('profileEmail', e.target.value)} />
              </div>
              <div className="form-row">
                <label>Bio</label>
              ...
              <div className="form-row">
                ...
                <div className="theme-picker">
                  {['dark', 'light', 'system'].map(t => (
                    <button key={t} className={`theme-btn ${settings.theme === t ? 'active' : ''}`} onClick={() => update('theme', t)}>
            ...
            <div className="settings-form">
              ...
              <div className="danger-zone">
                ...
                <div className="danger-row">
                  ...
                  <button className="btn btn-danger" onClick={() => setShowDeleteConfirm(true)}><Trash2 size={14} /> Delete Account</button>
                </div>
              </div>
            </div>
          </div>
              ...
              {apiKeys.map(k => (
                <div key={k.id} className="api-key-card">
                  ...
                  <div className="api-key-actions">
                    ...
                    <button className="icon-action" onClick={() => copyKey(k.key)}><Copy size={15} /></button>
                    <button className="icon-action" onClick={() => regenerateKey(k.id)}><RefreshCw size={15} /></button>
                    <button className="icon-action danger" onClick={() => deleteKey(k.id)}><Trash2 size={15} /></button>
                  </div>
                </div>
              ))}
        ...
        <div className="modal-overlay" onClick={() => setShowNewKeyModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="icon-close" onClick={() => setShowNewKeyModal(false)}><X size={18} /></button>
            </div>
            ...
            <div className="modal-actions">
              <button className="btn" onClick={() => setShowNewKeyModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={createKey} disabled={!newKeyName}><Key size={14} /> Create</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showResetConfirm && (
        <div className="modal-overlay" onClick={() => setShowResetConfirm(false)}>
          <div className="modal modal-sm" onClick={e => e.stopPropagation()}>
            ...
            <p className="confirm-text">This will restore all settings to their default values. This cannot be undone.</p>
            <div className="modal-actions">
              <button className="btn" onClick={() => setShowResetConfirm(false)}>Cancel</button>
              <button className="btn btn-danger" onClick={handleReset}><RefreshCw size={14} /> Reset</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <div className="modal modal-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-danger" onClick={() => { setShowDeleteConfirm(false); showToast('Account deletion requested') }}>
