...
import {
  Search, Plus, Download, Filter, Eye, Pencil, Trash2, X,
  ChevronLeft, ChevronRight, MoreHorizontal, Package, Truck,
  CreditCard, RefreshCw, Copy, CheckCircle,
} from 'lucide-react'
...
const emptyOrder = {
  id: '', customer: '', email: '', items: 1, amount: 0,
  status: 'pending', date: '', payment: 'credit_card', shipping: 'standard',
}
...
export default function Orders() {
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map(o =>
      `${o.id},${o.customer},${o.email},${o.items},${o.amount},${o.status},${o.date},${o.payment},${o.shipping}`
    )
    ...
    <div className="orders-page">
      ...
      <div className="page-header">
        ...
        <div className="header-actions">
          <button className="btn" onClick={exportCSV}><Download size={14} /> Export</button>
          <button className="btn btn-primary" onClick={() => setShowCreateModal(true)}><Plus size={14} /> New Order</button>
        </div>
      </div>
              ...
              <tr key={order.id} className={selectedOrders.has(order.id) ? 'selected' : ''}>
                ...
                <td>
                  <div className="row-actions">
                    <button className="icon-action" onClick={() => setShowDetail(order)}><Eye size={15} /></button>
                    <button className="icon-action" onClick={() => openEdit(order)}><Pencil size={15} /></button>
                    <div className="more-menu-wrapper">
                      <button className="icon-action" onClick={() => setActionMenu(actionMenu === order.id ? null : order.id)}>
                        <MoreHorizontal size={15} />
                      </button>
                      {actionMenu === order.id && (
                        <div className="action-dropdown">
                          {order.status === 'pending' && <button onClick={() => updateStatus(order.id, 'processing')}>Mark Processing</button>}
                          {order.status === 'processing' && <button onClick={() => updateStatus(order.id, 'shipped')}>Mark Shipped</button>}
                          {order.status === 'shipped' && <button onClick={() => updateStatus(order.id, 'completed')}>Mark Completed</button>}
        ...
        <div className="modal-overlay" onClick={() => setShowDetail(null)}>
          <div className="modal modal-wide" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="icon-close" onClick={() => setShowDetail(null)}><X size={18} /></button>
            </div>
            <div className="detail-grid">
              ...
              <div className="detail-item">
                ...
                <span className="detail-value"><span className={`status-badge ${showDetail.status}`}>{showDetail.status}</span></span>
              </div>
              <div className="detail-item">
            ...
            <div className="modal-actions">
              <button className="btn" onClick={() => { setShowDetail(null); openEdit(showDetail) }}>
      ...
      {showCreateModal && (
        <div className="modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>New Order</h2>
              <button className="icon-close" onClick={() => setShowCreateModal(false)}><X size={18} /></button>
            </div>
            ...
            <div className="form-row-inline">
              <div className="form-group">
                <label>Items</label>
                <input type="number" min="1" value={newOrder.items} onChange={e => setNewOrder({ ...newOrder, items: +e.target.value })} />
              </div>
              <div className="form-group">
                <label>Amount ($)</label>
                <input type="number" min="0" step="0.01" value={newOrder.amount} onChange={e => setNewOrder({ ...newOrder, amount: +e.target.value })} />
              </div>
            </div>
            <div className="form-row-inline">
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <select value={newOrder.shipping} onChange={e => setNewOrder({ ...newOrder, shipping: e.target.value })}>
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-primary" onClick={createOrder} disabled={!newOrder.customer || !newOrder.email}>
        ...
        <div className="modal-overlay" onClick={() => setShowEditModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="icon-close" onClick={() => setShowEditModal(false)}><X size={18} /></button>
            </div>
            <div className="form-group">
              <label>Customer Name</label>
            ...
            <div className="form-row-inline">
              <div className="form-group">
                <label>Items</label>
                <input type="number" min="1" value={editOrder.items} onChange={e => setEditOrder({ ...editOrder, items: +e.target.value })} />
              </div>
              <div className="form-group">
                <label>Amount ($)</label>
                <input type="number" min="0" step="0.01" value={editOrder.amount} onChange={e => setEditOrder({ ...editOrder, amount: +e.target.value })} />
              </div>
            </div>
            <div className="form-row-inline">
              <div className="form-group">
            ...
            <div className="form-group">
              <label>Shipping</label>
              <select value={editOrder.shipping} onChange={e => setEditOrder({ ...editOrder, shipping: e.target.value })}>
                <option value="standard">Standard</option>
                <option value="express">Express</option>
                <option value="overnight">Overnight</option>
              </select>
